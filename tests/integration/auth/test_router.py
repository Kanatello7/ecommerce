...
async def test_register_success(async_client):
    ...
    assert (
        user["first_name"] == "Kanat"
        and user["last_name"] == "Zhetru"
        and user["email"] == "Kanat123@gmail.com"
    )
...
async def test_refresh_token_success(async_client):
    ...
    refresh_response = await async_client.post(
        url="/auth/refresh", cookies={"refresh_token": old_refresh_token}
    )
...
async def test_refresh_token_expired(async_client):
    ...
    expired_token = jwt.encode(
        exp_payload, settings_jwt.SECRET_KEY, algorithm=settings_jwt.ALGORITHM
    )

    response = await async_client.post(
        url="/auth/refresh", cookies={"refresh_token": expired_token}
    )
...
async def test_refresh_token_invalid(async_client):
    ...
    response = await async_client.post(
        url="/auth/refresh", cookies={"refresh_token": "invalid_token_string"}
    )
...
async def test_logout(async_client):
    response = await async_client.post(
        url="/auth/logout", headers={"accept": "application/json"}
    )
    ...
    access_cookie_deleted = any(
        "access_token" in cookie and ("Max-Age=0" in cookie or "expires=" in cookie)
        for cookie in set_cookie_headers
    )
    refresh_cookie_deleted = any(
        "refresh_token" in cookie and ("Max-Age=0" in cookie or "expires=" in cookie)
        for cookie in set_cookie_headers
    )
    ...
    assert (
        access_cookie_deleted or len(set_cookie_headers) >= 1
    )  # Cookie deletion sets headers
